...
app.post('/api/data', function (req, res) {
    ...
    console.log('value', req.body.metadata.fileName);
    var fileNamePass = req.userId + '-' + req.body.metadata.fileName;
        ...
        var p = config.getDB().then(function (db) {
            db.collection("Datametadata").findOne(query, function (err, data) {
                ...
                if (err) {
                    ...
                    console.log('dataaaaname');
                    if (data != null) {
                        res.status(200).json({ success: false, message: 'file already exist' })
                    }
                    else {
                        fs.writeFile('file/' + fileNamePass, JSON.stringify(req.body.data), "utf8", function (err, data) {
                            if (err) {
                                console.log(err);
                            }
                            else {
...
// app.get('/api/data/:id', function (req, res) {
    app.get('/api/data', function (req, res) {
    console.log('datazzz', res.query.ids);
    var fileNamePass = req.userId + '-' + req.query.ids;
    fs.readFile('file/' + fileNamePass, function (err, data) {
        var data1 = JSON.parse(data);
        console.log('zz', req.userId);
        if (data1) {
            res.status(200).json({ success: true, message: 'retrieved data successfully', data: JSON.parse(data) })
...
app.put('/api/data/:id', function (req, res) {
   var fileNamePass =res.userId+'-'+res.param.id;
   console.log('datazzz'+fileNamePass);
    var createdate = new Date();
